...
const SearchSection = ({ searchBoxBackground = '#fff', openFromRight = false, borderColor }: SearchSectionProps): JSX.Element => {
  const theme = useTheme();
      ...
      <Box sx={{ display: { xs: 'block', md: 'none' } }}>
        <PopupState variant="popper" popupId="demo-popup-popper">
          {(popupState) => (
            <>
              <Box sx={{ ml: 2 }}>
                <ButtonBase sx={{ borderRadius: '30%', border: borderColor ? `2px solid ${borderColor}` : '0' }}>
              ...
              <PopperStyle {...bindPopper(popupState)} transition>
                {({ TransitionProps }) => (
                  <>
                    <Transitions type="zoom" {...TransitionProps} sx={{ transformOrigin: `center ${openFromRight ? 'right' : 'left'}` }}>
                      <Card
                        sx={{
